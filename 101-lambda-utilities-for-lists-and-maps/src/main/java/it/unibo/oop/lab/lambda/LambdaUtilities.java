...
public final class LambdaUtilities {
    ...
    public static <T> List<Optional<T>> optFilter(final List<T> list, final Predicate<T> pre) {
         ...
         */
        final List<Optional<T>> results = new ArrayList<>();
        list.forEach(t -> results.add(Optional.ofNullable(t).filter(pre)));
        return results;
    }
    ...
    public static <R, T> Map<R, Set<T>> group(final List<T> list, final Function<T, R> op) {
         ...
         */
        final Map<R, Set<T>> results = new HashMap<>();
        ...
        list.forEach(t -> results.merge(op.apply(t), Set.of(t), union));
        return results;
    }
    ...
    public static <K, V> Map<K, V> fill(final Map<K, Optional<V>> map, final Supplier<V> def) {
        ...
        map.forEach((k, v) -> res.put(k, v.orElse(def.get())));
        return res;
    }
